...
public class EntityHtmlView extends HtmlView {
  ...
  public EntityHtmlView(String entityName, String entityId) {
  ...
  protected void buildHtml() throws IOException {
    ...
    if (EL.isBindVariable(mEntityId)) {
      ...
      entityId = Long.valueOf(mEntityId);
    }
    ...
    Object entity = mEntityDataProvider.getEntity(mEntityName, entityId);

    setVariable(mVariable, entity);

    // TODO workaround is for entityForm
    if (getParent() != null) {
      getParent().setVariable(mVariable, entity);
    }
  }
}
